    ...
    <NetworkContext.Provider value={{ isOffline, notifyOffline, cacheData, getCachedData }}>
      {showOfflineToast && (
        <Animated.View
          style={[
            styles.toastBase,
            toastType === 'offline' ? styles.offlineToast : styles.onlineToast,
            {
        ...
        >
          <Ionicons
            name={toastType === 'offline' ? 'cloud-offline-outline' : 'cloud-done-outline'}
            size={16}
            color="#FFFFFF"
          />
          <Text style={styles.offlineText}>
            {toastType === 'offline' ? 'No tienes conexión a internet.' : 'Ya tienes conexión a internet.'}
          </Text>
        </Animated.View>
      )}
      {children}
...
const styles = StyleSheet.create({
  toastBase: {
    position: 'absolute',
    top: Platform.OS === 'android' ? (StatusBar.currentHeight || 0) + 8 : 56,
    left: 16,
    right: 16,
    zIndex: 9999,
    paddingVertical: 12,
